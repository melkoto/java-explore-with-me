...
public interface AdminEventRepository extends JpaRepository<Event, Long> {
    @Query("SELECT e FROM Event e WHERE (e.initiator.id IN :userIds OR :userIds IS NULL) " +
            "AND (e.state IN :states OR :states IS NULL) AND (e.category.id IN :categories OR :categories IS NULL) " +
            "AND (e.eventDate BETWEEN :fromDate AND :toDate)")
